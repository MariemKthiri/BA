...
function transform(iAntenna, index, x)
    s = size(x)
    if length(s)>2
        return reshape(pinv(matrices[iAntenna][index,:,:])*squeeze(x,2),2*s[1],s[2],s[3])
    else
    ...
    for index in 1:10
        ...
        # Network estimators
            ...
            nn_est[:KIdSoftmax] = CondNormalKnet.ConvNN(nLayers, Int(nAntennas/2),
                                            ...
                                            activation = CondNormalKnet.softmax)
        #=else
            nn_est[:KCircReLU].inTransform = (x) -> circ_trans(transform(iAntenna, index, x),:notransp)
            nn_est[:KToepReLU].inTransform = (x) -> toep_trans(transform(iAntenna, index, x),:notransp)
            nn_est[:KIdReLU].inTransform = (x) -> (transform(iAntenna, index, x),:notransp)
            nn_est[:KCircSoftmax].inTransform = (x) -> circ_trans(transform(iAntenna, index, x),:notransp)
            nn_est[:KToepSoftmax].inTransform = (x) -> toep_trans(transform(iAntenna, index, x),:notransp)
            nn_est[:KIdSoftmax].inTransform = (x) -> (transform(iAntenna, index, x),:notransp)

            CondNormalKnet.resize!(nn_est[:KCircReLU],    Int(nAntennas/2),
                                    inTransform  = (x) -> circ_trans(transform(iAntenna, index, x),:notransp),
                                    outTransform = (x) -> x, learning_rate = learning_rates_relu[iAntenna])
            CondNormalKnet.resize!(nn_est[:KToepReLU],    Int(nAntennas/2),
                                    inTransform  = (x) -> toep_trans(transform(iAntenna, index, x),:notransp),
                                    outTransform = (x) -> x, learning_rate = learning_rates_relu[iAntenna])
            CondNormalKnet.resize!(nn_est[:KIdReLU]  ,    Int(nAntennas/2),
                                    inTransform  = (x) -> (transform(iAntenna, index, x)),
                                    outTransform = (x) -> x, learning_rate = learning_rates_relu[iAntenna])
            CondNormalKnet.resize!(nn_est[:KCircSoftmax], Int(nAntennas/2),
                                    inTransform  = (x) -> circ_trans(transform(iAntenna, index, x),:notransp),
                                    outTransform = (x) -> x, learning_rate = learning_rates_relu[iAntenna])
            CondNormalKnet.resize!(nn_est[:KToepSoftmax], Int(nAntennas/2),
                                    inTransform  = (x) -> toep_trans(transform(iAntenna, index, x),:notransp),
                                    outTransform = (x) -> x, learning_rate = learning_rates_relu[iAntenna])
            CondNormalKnet.resize!(nn_est[:KIdSoftmax]  , Int(nAntennas/2),
                                    inTransform  = (x) -> (transform(iAntenna, index, x)),
                                    outTransform = (x) -> x, learning_rate = learning_rates_relu[iAntenna])       
        end=#
...
for iAntenna in 1:3
    ...
    for index in 1:10
        ...
        for bb in 1:nBatches
            ...
            y = reshape(matrix * squeeze(h, 2), Int(nAntennas/2),nCoherence,nBatches) + 10^(-snr/20) * crandn( Int(nAntennas/2),nCoherence,nBatches)
            hest = zeros(Complex128, nAntennas,nCoherence,nBatches)
            for j=1:nBatchSize ,t=1:nCoherence
                    (hest[:,t,j],_) = OMP(y[:,t,j],(size(h)[1]),m,Int(nAntennas/2),matrix)
