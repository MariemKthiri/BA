...
for iAntenna in 1:3
    ...
    #for n = 1:nLearningBatches/500
        ...
        train_batch!(nn_est, snr = snr, nBatches = nLearningBatches, get_channel = () -> get_channel2(sparsity, nAntennas, nCoherence, nLearningBatchSize, seed), verbose = verbose)
        ...
        (errs,rates) = evaluate(algs, snr = snr, nBatches = nBatches, get_channel = () -> get_channel2(sparsity, nAntennas, nCoherence, nBatchSize, 2), verbose = verbose)
...
#for i in 1:1000
    for bb in 1:nBatches
        ...
        for j=1:nBatchSize
            for t=1:nCoherence
                (hest[:,t,j],supp_est[:,j]) = OMP(y[:,t,j],size(h)[1],Int(m),N)
...
#-------------------------------------------------
CSV.write("/home/mariem/BA/MSE = f(nAntennas) (another copy)/SparseRecovery_sparsity_$(m)_snr_$(snr)_dB_prob_$(100*sparsity).csv", results)

#CSV.write("/home/mariem/BA/Learning Curves/LearningCurves_sparsity_$(m)_snr_$(snr)_dB.csv", results)

CSV.write("/home/mariem/BA/Sparsity Tests (another copy)/test_sparsity_$(m)_snr_$(snr)_dB_prob_$(100*sparsity).csv", test_results)

CSV.write("/home/mariem/BA/Support Tests (another copy)/supp_test_sparsity_$(m)_snr_$(snr)_dB_prob_$(100*sparsity).csv", supp_results)
...
for (alg,est) in nn_est cntf.save(est, "/home/mariem/BA/Neural Networks/$(String(alg))_copy3.jl") end

plot_8_antennas = DataFrame()
plot_8_antennas = results[results[:nAntennas].==8,[:MSE,:rate,:Algorithm,:SNR,:nAntennas,:nCoherence]]
CSV.write("/home/mariem/BA/MSE = f(SNR)/sparsity_$(m)_8_antennas_prob.csv", plot_8_antennas)

plot_16_antennas = DataFrame()
plot_16_antennas = results[results[:nAntennas].==16,[:MSE,:rate,:Algorithm,:SNR,:nAntennas,:nCoherence]]
CSV.write("/home/mariem/BA/MSE = f(SNR)/sparsity_$(m)_16_antennas_prob.csv", plot_16_antennas)

plot_32_antennas = DataFrame()
plot_32_antennas = results[results[:nAntennas].==32,[:MSE,:rate,:Algorithm,:SNR,:nAntennas,:nCoherence]]
CSV.write("/home/mariem/BA/MSE = f(SNR)/sparsity_$(m)_32_antennas_prob.csv", plot_32_antennas)


plot_8_antennas = DataFrame()
plot_8_antennas = results[results[:nAntennas].==8,[:MSE,:rate,:Algorithm,:SNR,:nAntennas,:nCoherence]]
CSV.write("/home/mariem/BA/MSE = f(sparsity)/snr_$(snr)_8_antennas_prob.csv", plot_8_antennas)

plot_16_antennas = DataFrame()
plot_16_antennas = results[results[:nAntennas].==16,[:MSE,:rate,:Algorithm,:SNR,:nAntennas,:nCoherence]]
CSV.write("/home/mariem/BA/MSE = f(sparsity)/snr_$(snr)_16_antennas_prob.csv", plot_16_antennas)

plot_32_antennas = DataFrame()
plot_32_antennas = results[results[:nAntennas].==32,[:MSE,:rate,:Algorithm,:SNR,:nAntennas,:nCoherence]]
CSV.write("/home/mariem/BA/MSE = f(sparsity)/snr_$(snr)_32_antennas_prob.csv", plot_32_antennas)
