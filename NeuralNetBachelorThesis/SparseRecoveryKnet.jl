...
for iAntenna in 1:3
    ...
    matrices[iAntenna] = Array{Complex128}(50,nAntennas,nAntennas)
    for i in 1:50
        (matrices[iAntenna][i,:,:] = get_matrix(nAntennas, nAntennas))
    end
end
...
for iAntenna in 1:3
    ...
    #for index in 1:10
        ...
        if iAntenna == 1
            # Knet-based networks
            nn_est[:KCircReLU] = CondNormalKnet.ConvNN(nLayers, nAntennas,
            ...
            nn_est[:KToepReLU] = CondNormalKnet.ConvNN(nLayers, nAntennas,
                                            inTransform  = (x) -> toep_trans(transform(iAntenna, index, x),:notransp),
                                            outTransform = (x) -> toep_trans(x,:transp),
                                            learning_rate = learning_rates_relu[iAntenna])
            nn_est[:KIdReLU] = CondNormalKnet.ConvNN(nLayers, nAntennas,
                                            inTransform  = (x) -> transform(iAntenna, index, x),
                                            outTransform = (x) -> x,
                                            learning_rate = learning_rates_relu[iAntenna])
            nn_est[:KCircSoftmax] = CondNormalKnet.ConvNN(nLayers, nAntennas,
                                            inTransform  = (x) -> circ_trans(transform(iAntenna, index, x),:notransp),
                                            outTransform = (x) -> circ_trans(x,:transp),
                                            learning_rate = learning_rates_relu[iAntenna],
                                            activation = CondNormalKnet.softmax)
            nn_est[:KToepSoftmax] = CondNormalKnet.ConvNN(nLayers, nAntennas,
                                            inTransform  = (x) -> toep_trans(transform(iAntenna, index, x),:notransp),
                                            outTransform = (x) -> toep_trans(x,:transp),
                                            learning_rate = learning_rates_relu[iAntenna],
                                            activation = CondNormalKnet.softmax)
            nn_est[:KIdSoftmax] = CondNormalKnet.ConvNN(nLayers, nAntennas,
                                            inTransform  = (x) -> transform(iAntenna, index, x),
                                            outTransform = (x) -> x,
                                            learning_rate = learning_rates_relu[iAntenna],
                                            activation = CondNormalKnet.softmax)
            ...
            CondNormalKnet.resize!(nn_est[:KIdReLU]  ,    nAntennas, learning_rate = learning_rates_relu[iAntenna])
            ...
            CondNormalKnet.resize!(nn_est[:KIdSoftmax]  , nAntennas, learning_rate = learning_rates_relu[iAntenna])       
        end
...
for iAntenna in 1:3
    ...
    #for index in 1:10
        ...
        for bb in 1:nBatches
            ...
            y = reshape(matrix * squeeze(h, 2), nAntennas,nCoherence,nBatches) + 10^(-snr/20) * crandn( nAntennas,nCoherence,nBatches)
            hest = zeros(Complex128, nAntennas,nCoherence,nBatches)
            for j=1:nBatchSize ,t=1:nCoherence
                    (hest[:,t,j],_) = OMP(y[:,t,j],(size(h)[1]),m,nAntennas,matrix)
